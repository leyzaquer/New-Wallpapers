...
class BitmapCropTask {
    ...
    // ... other fields
    int mDestination; // Add this field

    // Update constructors to accept the destination
    public BitmapCropTask(Context c, Resources res, int inResId,
                          RectF cropBounds, int rotation, int outWidth, int outHeight,
                          boolean setWallpaper, boolean saveCroppedBitmap, Runnable onEndRunnable, int destination) { // Add destination
        mContext = c;
        mInResId = inResId;
        mResources = res;
        mDestination = destination; // Set destination
    ...
    public boolean cropBitmap() {
        ...
        if (mInStream != null) {
            ...
            if (mRotation > 0) {
                ...
                mCropBounds.offset((float) bounds.x /2, (float) bounds.y /2);
            ...
            // mOnBitmapCroppedHandler.onBitmapCropped(mCroppedBitmap);
        ...
        if (mSetWallpaper && wallpaperManager != null) {
            try {
                // Use the modern setBitmap method with destination flags
                wallpaperManager.setBitmap(crop, null, true, mDestination);
            } catch (IOException e) {
                Log.w(LOGTAG, "cannot write stream to wallpaper", e);
                failure = true;
            }
    ...
    public void execute() {
        executor.execute(() -> {
            final boolean result = cropBitmap();
            handler.post(() -> {
